...
class ErlangCodegen(SemanticAnalyzer):
    ...
    def __init__(self, ast, module_name="module_name", external_functions=None):
        ...
        self.pattern_binding_bases = set()
    ...
    def generate_match_clause(self, clause: MatchClause, merge_vars, start_versions):
        ...
        for binding in sorted(bindings):
            base_name = self.emit_local_name(binding)
            if (
                binding in prev_locals
                or binding in self.global_var_names
                or base_name in self.pattern_binding_bases
            ):
                self.pattern_binding_counter += 1
                binding_scope[binding] = (
                    f"{base_name}_Match{self.pattern_binding_counter}"
                )
            else:
                binding_scope[binding] = base_name
            self.pattern_binding_bases.add(base_name)
